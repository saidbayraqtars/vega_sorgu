...
const DEF = [
  ...
  { id: "ozel.nakit_projeksiyonu", ad: "Nakit Projeksiyonu (90 gün)", kisa: "Projeksiyon", ikon: "Telescope", kategori: "durum", tur: "projeksiyon", grafik: "cizgi", grafikler: ["cizgi"],
    aciklama: "Vade takvimine göre kesin ve geçmiş ortalamalara göre beklenen nakit seyri.", fn: (ctx, prm, q) => ({ tur: "projeksiyon", ...projection(ctx, { gun: Number(q.gun) || 90 }) }) },
  ...
  { id: "ozel.alacak_yaslandirma", ad: "Alacak Yaşlandırma", kisa: "Yaşlandırma", ikon: "AlarmClock", kategori: "tahsilat", tur: "coklu", grafik: "coklu",
    aciklama: "Açık alacaklar vadesine göre (FIFO: bakiye en yeni faturalara dağıtılır).", fn: (ctx, prm) => {
      ...
      return { tur: "coklu", parcalar: [
        ...
        tablo([K("ad", "Müşteri", "metin"), K("bakiye", "Bakiye"), K("gelmemis", "Gelmemiş"), K("g0_30", "1-30"), K("g31_60", "31-60"), K("g61_90", "61-90"), K("g90", "90+"), K("gecikme", "En eski gecikme (gün)", "sayi")],
          a.cariler.slice(0, prm.n * 5), { baslik: "Müşteri bazında" }),
      ] };
    } },
  { id: "ozel.borclu_musteriler", ad: "Borçlu Müşteriler", kisa: "Borçlular", ikon: "UserMinus", kategori: "tahsilat", tur: "tablo", grafik: "tablo",
    aciklama: "Bize borcu olan cariler (aktif dönem bakiyesi, KREDIHESABI ve personel hariç).", fn: (ctx, prm) => {
      const c = B.currentCari(ctx).list.filter((x) => x.bakiye > 0).sort((a, b) => b.bakiye - a.bakiye).slice(0, prm.n * 10);
      return tablo([K("ad", "Müşteri", "metin"), K("kod", "Kod", "metin"), K("il", "İl", "metin"), K("bakiye", "Bakiye"), K("kredi_limit", "Limit"), K("son", "Son hareket", "tarih")], c);
    } },
  { id: "ozel.alacakli_tedarikciler", ad: "Borçlu Olduğumuz Cariler", kisa: "Borçlarımız", ikon: "UserPlus", kategori: "alis", tur: "tablo", grafik: "tablo",
    aciklama: "Tedarikçilere olan borçlar (eksi bakiyeler).", fn: (ctx, prm) => {
      const c = B.currentCari(ctx).list.filter((x) => x.bakiye < 0).sort((a, b) => a.bakiye - b.bakiye).slice(0, prm.n * 10).map((x) => ({ ...x, borc: -x.bakiye }));
      return tablo([K("ad", "Cari", "metin"), K("kod", "Kod", "metin"), K("borc", "Borcumuz"), K("son", "Son hareket", "tarih")], c);
    } },
  { id: "ozel.hareketsiz_alacak", ad: "Hareketsiz Alacaklar", kisa: "Hareketsiz", ikon: "Snail", kategori: "tahsilat", tur: "tablo", grafik: "tablo",
    aciklama: "90 günden uzun süredir hareketi olmayan ama bakiyesi olan müşteriler.", fn: (ctx) => {
      const t = ctx.today;
      ...
      return tablo([K("ad", "Müşteri", "metin"), K("bakiye", "Bakiye"), K("son", "Son hareket", "tarih"), K("gun", "Geçen gün", "sayi")], c);
    } },
  { id: "ozel.limit_asimi", ad: "Kredi Limiti Aşımı", kisa: "Limit aşımı", ikon: "ShieldAlert", kategori: "tahsilat", tur: "tablo", grafik: "tablo",
    aciklama: "Bakiyesi cari kartındaki kredi limitini aşan müşteriler.", fn: (ctx) => {
      ...
      return tablo([K("ad", "Müşteri", "metin"), K("bakiye", "Bakiye"), K("kredi_limit", "Limit"), K("asim", "Aşım"), K("oran", "Aşım oranı", "yuzde")], c);
    } },
  ...
  ...[["cek", "alinan", "Alınan Çekler (Portföy)", "Ticket"], ["cek", "verilen", "Verilen Çekler (Ödenecek)", "TicketX"],
    ...
    aciklama: `${ad}: yalnız açık olanlar (VARES görünümü; alınanlarda 'Tahsilat Yok', verilenlerde 'Ödenecek').`, fn: (ctx) => {
      const t = ctx.today;
      ...
      return tablo([K("vade", "Vade", "tarih"), K("gun", "Kalan gün", "sayi"), K("tutar", "Tutar"), K("cari", "Cari", "metin"), K("belgeno", "No", "metin"), K("banka", "Banka", "metin"), K("durum", "Durum", "etiket")],
        rows, { toplam: { tutar: rows.reduce((s, r) => s + r.tutar, 0) } });
    },
  })),
  ...
  { id: "ozel.rfm", ad: "Müşteri Segmentleri (RFM)", kisa: "Segmentler", ikon: "Crown", kategori: "musteri", tur: "coklu", grafik: "coklu",
    aciklama: "Son alış zamanı, alış sıklığı ve tutarına göre şampiyon, sadık, risk altında, uykuda… müşteriler.", fn: (ctx, prm) => {
      ...
      return { tur: "coklu", parcalar: [
        ...
        tablo([K("ad", "Müşteri", "metin"), K("segmentAd", "Segment", "etiket"), K("sonAlis", "Son alış", "tarih"), K("gunOnce", "Gün önce", "sayi"), K("fatura", "Fatura", "sayi"), K("ciro", "Ciro (12 ay)")],
          r.musteriler.slice(0, prm.n * 10), { baslik: "Müşteriler" }),
      ] };
    } },
  { id: "ozel.yeni_kayip", ad: "Yeni, Kayıp ve Geri Dönen Müşteriler", kisa: "Yeni/kayıp", ikon: "UserRoundCheck", kategori: "musteri", tur: "coklu", grafik: "coklu", donem: "bu_ay",
    aciklama: "Dönemde ilk kez alan, önceki dönemde alıp bu dönemde almayan ve uzun aradan sonra dönen müşteriler.", fn: (ctx, prm) => {
      ...
      const tb = (list, baslik) => tablo([K("ad", "Müşteri", "metin"), K("v", "Ciro")], list.slice(0, prm.n * 3), { baslik });
      return { tur: "coklu", parcalar: [
  ...
  { id: "ozel.zararli_urunler", ad: "Zararına Satılan Ürünler", kisa: "Zararlı ürünler", ikon: "TriangleAlert", kategori: "karlilik", tur: "tablo", grafik: "tablo", donem: "son90",
    aciklama: "Satış tutarı maliyetinin altında kalan ürünler.", fn: (ctx, prm) => {
      ...
      return tablo(profitCols("Ürün"), t);
    } },
  { id: "ozel.urun_matrisi", ad: "Ürün Matrisi (Büyüme × Marj)", kisa: "Ürün matrisi", ikon: "ScatterChart", kategori: "urun", tur: "tablo", grafik: "tablo", donem: "son90",
    aciklama: "Her ürün için satış büyümesi (geçen yılın aynı dönemine göre) ve brüt marj: yıldızlar, nakit inekleri, sorunlular.", fn: (ctx, prm) => {
      ...
      const rows = cur.filter((r) => r.v > 0).slice(0, 200).map((r) => {
      ...
      return tablo([K("ad", "Ürün", "metin"), K("satis", "Net satış"), K("buyume", "Büyüme", "yuzde"), K("marj", "Marj", "yuzdeSayi"), K("grup", "Grup", "etiket")], rows);
    } },
  ...
  { id: "ozel.eksi_stok", ad: "Eksi Stoklu Ürünler", kisa: "Eksi stok", ikon: "PackageX", kategori: "stok", tur: "tablo", grafik: "tablo",
    aciklama: "Depo envanteri eksiye düşmüş ürünler — giriş belgesi eksik olabilir.", fn: (ctx) => tablo(stockCols(), stockRows(ctx).filter((r) => r.miktar < 0).sort((a, b) => a.miktar - b.miktar)) },
  { id: "ozel.kritik_stok", ad: "Kritik Seviyenin Altındakiler", kisa: "Kritik stok", ikon: "PackageMinus", kategori: "stok", tur: "tablo", grafik: "tablo",
    aciklama: "Kalanı kart kritik seviyesinin altında olan (sipariş verilmesi gereken) ürünler.", fn: (ctx) => tablo([...stockCols(), K("kritik", "Kritik", "sayi"), K("eksik", "Sipariş", "sayi")],
      stockRows(ctx).filter((r) => r.kritik > 0 && r.miktar < r.kritik).map((r) => ({ ...r, eksik: r.kritik - r.miktar })).sort((a, b) => b.eksik - a.eksik)) },
  { id: "ozel.olu_stok", ad: "Ölü Stok", kisa: "Ölü stok", ikon: "Skull", kategori: "stok", tur: "tablo", grafik: "tablo",
    aciklama: "Stoğu olduğu hâlde son 180 günde hiç satılmamış ürünler.", fn: (ctx) => {
      const t = ctx.today;
      const sold = new Set(Q.byDim(ctx, "miktar", "urun", { bas: P.addDays(t, -179), bit: t, limit: 0 }).satirlar.filter((r) => r.v > 0).map((r) => r.k));
      return tablo(stockCols(), stockRows(ctx).filter((r) => r.miktar > 0 && !sold.has(`${r.firma}:${r.id}`)).sort((a, b) => b.deger - a.deger));
    } },
  { id: "ozel.stok_devir", ad: "Stok Devir Hızı", kisa: "Devir hızı", ikon: "RefreshCw", kategori: "stok", tur: "tablo", grafik: "tablo",
    aciklama: "Son 90 günün satış miktarına göre kaç günlük stok olduğu ve devir hızı.", fn: (ctx) => {
      const t = ctx.today;
      ...
      return tablo([...stockCols(), K("satis90", "90 gün satış", "sayi"), K("gunSayisi", "Kaç günlük", "sayi"), K("devir", "Devir hızı (90g)", "sayi")], rows);
    } },
...
function profitTable(ctx, prm, dim, ad) {
  ...
  return tablo(profitCols(ad), rows.slice(0, prm.n * 10), {
...
function abcReport(ctx, prm, olcu, boyut, ad) {
  ...
  return { tur: "coklu", parcalar: [
    ...
    tablo([K("ad", ad, "metin"), K("v", "Tutar"), K("pay", "Pay", "yuzde"), K("kum", "Kümülatif", "yuzde"), K("sinif", "Sınıf", "etiket")], a.satirlar.slice(0, prm.n * 10), { baslik: `${ad} listesi` }),
  ] };
}
function stockCols() {
...
const LIST = DEF.map(({ fn, ...meta }) => ({ ...meta, donem: meta.donem || "bu_ay", birim: meta.birim || "TL" }));
...
module.exports = { LIST, run, IZAHAT_AD };
